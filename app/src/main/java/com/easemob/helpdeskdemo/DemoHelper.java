...
//                    UserUtil.setAgentNickAndAvatar(context, message, userAvatarView, usernickView);
                    ...
                    if (userAvatarView != null){
                        userAvatarView.setImageResource(R.mipmap.img_custom_service_icon);
                        if (agentInfo != null){
                            if (!TextUtils.isEmpty(agentInfo.getAvatar())) {
                                ...
                                if (!TextUtils.isEmpty(strUrl)) {
                                    ...
                                    //正常的string路径
                                    Glide.with(context).load(strUrl).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(R.mipmap.img_custom_service_icon).transform(new GlideCircleTransform(context)).into(userAvatarView);
                                }
                            }
                        }
                    ...
                    //此处设置当前登录用户的头像，
                    if (userAvatarView != null){
                        if (Constants.loginresultInfo.getUser().getUserId()!=null && Constants.loginresultInfo.getUser().getImage()!=null && !Constants.loginresultInfo.getUser().getImage().equals("")) {
                            ...
                            if(bitmap==null){
                                ...
                                Bitmap b = ImageUtils.getRoundBitmap(bitmap, 20);
                                userAvatarView.setImageBitmap(b);
                            }
                        }else {
                            userAvatarView.setImageResource(R.drawable.hd_default_avatar);
                        }
